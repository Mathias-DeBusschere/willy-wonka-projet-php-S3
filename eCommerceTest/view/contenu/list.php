...
foreach ($tab_contenu as $contenu){
		echo '<article class="other_product_article" style="flex-grow:1">
		 IdCommande: <a href="index.php?controller=commande&action=read&id='.rawurlencode($contenu->getIdCommande()).'">'.$contenu->getIdCommande().'</a> | IdProduit: 
		<a href="index.php?controller=chocolat&action=read&id='.rawurlencode($contenu->getIdProduit()).'">'.$contenu->getIdProduit().'</a> | Quantité: '
		.$contenu->getQuantite().' | '
		.
